...
let startButton = document.querySelector("#boutonstart")
...
function start() {
  ...
  motion = setInterval(function () {
    document.querySelectorAll("#enemies > .eagle .isaac").forEach(function (div) {
      move(div);
    });
  }, motionDelay);
...
function spawnEnemy() {
  ...
  newObstacle.style.bottom = getRandomArbitrary(1, 900) + "px";
  enemies.appendChild(newObstacle);

  
  let secondObstacle = document.createElement("div");
  secondObstacle.classList.add("isaac");
  secondObstacle.style.bottom = getRandomArbitrary(1, 900) + "px";
  enemies.appendChild(secondObstacle);

}

let intervalID = setInterval(spawnEnemy, 1000);
